	...
	else if( m_pQuatPosKeys )
	{
		...
		if( !m_pQuatOrigRot )
		{
			...
			for( DWORD i=0; i<m_cQuatPosKeys; ++i )
			{
				...
				D3DXQuaternionMultiply( &m_pQuatOrigRot[i], &vQuat, &vQuatInv );	// ���� ȸ���̴�.
			}
		}
...
void SAnimation::ConvertMatToQuat()
{
	...
	if( !m_pQuatPosKeys && m_pMatrixKeys )
	{
		...
		for( DWORD i=0; i<m_cQuatPosKeys; ++i )
		{
			...
			CompressionQuaternion( m_pQuatPosKeys[i].m_vQuatComp, vQuat );	// ����� ����
		}
